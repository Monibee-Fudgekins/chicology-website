...
(async () => {
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (!init || init.method === void 0 || init.method === "GET") {
      const match2 = url.match(/\/issues\/(\d+)$/);
      const issueNumber2 = match2 ? parseInt(match2[1], 10) : 123;
      const response2 = new Response(JSON.stringify({ number: issueNumber2, state: "open" }), { status: 200 });
      console.log("FETCH RESPONSE (GET):", { issueNumber: issueNumber2, state: "open" });
      return response2;
    }
